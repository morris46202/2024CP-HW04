        ...
        else
        {
            ...
            while((ptr = readdir(dir)) != NULL)
            {
                if(ptr->d_name[0] >= '0' && ptr->d_name[0] <= '9')
                {
                    ...
                    printf("%d     %s      %c      %lu     %ld\n", cpu_data.pid, ptr->d_name, cpu_data.state, cpu_usage, cpu_data.vsize);
                }
            }
            closedir(dir);
        }
...
rd get_cpu_data( int32_t mypid )
{
    ...
    int32_t priority, nice, num_threads, itrealvalue;
    u_int64_t starttime;
    u_int32_t vsize;
    
    fscanf(fp, "%d %s %c %d %d %d %d %d %u %lu %lu %lu %lu %lu %lu %ld %ld, %ld %ld %d %d %d %d %u %llu %lu",
